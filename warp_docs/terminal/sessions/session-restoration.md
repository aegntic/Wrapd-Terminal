# https://docs.wrapd.dev/terminal/sessions/session-restoration

**Crawled on:** 16139.799568864
...
On this page
  * [What is it](https://docs.wrapd.dev/terminal/sessions/session-restoration#what-is-it)
  * [How to access Session Restoration](https://docs.wrapd.dev/terminal/sessions/session-restoration#how-to-access-session-restoration)
  * [How Session Restoration works](https://docs.wrapd.dev/terminal/sessions/session-restoration#how-session-restoration-works)
...
# Session Restoration
The Session Restoration feature enables Wrapd to restore your session history, specifically windows, tabs, and panes, along with the last few Blocks in each pane.
## 
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#what-is-it)
What is it
Session restoration allows you to quickly pick up where you left off in your previous terminal session.
## 
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#how-to-access-session-restoration)
How to access Session Restoration
  * Session Restoration comes enabled by default in Wrapd.
...
Toggling off Session Restoration will not clear the [SQLite database](https://docs.wrapd.dev/terminal/sessions/session-restoration#session-restoration-database); however, Wrapd will stop recording new output.
## 
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#how-session-restoration-works)
How Session Restoration works
![](https://docs.wrapd.dev/~gitbook/image?url=https%3A%2F%2F2297236823-files.gitbook.io%2F%7E%2Ffiles%2Fv0%2Fb%2Fgitbook-x-prod.appspot.com%2Fo%2Fspaces%252F-MbqIgTw17KQvq_DQuRr%252Fuploads%252Fgit-blob-eea5d549c432c9c124c175120bc2b901b1add9fb%252Fsessions-block_restoration.gif%3Falt%3Dmedia%26token%3D56d16d7b-d27f-4d3d-b0af-a5ff017b5ead&width=768&dpr=4&quality=100&sign=854d44db&sv=2)
Session Restoration Demo
#### 
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#session-restoration-database)
Session Restoration database
Wrapd saves the data from your previous session's windows, tabs, and panes to a SQLite database on your computer, and every time you quit the app, this data is overwritten by your latest session. You can open the database directly and inspect its full contents like so:
macOS
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#tab-macos)
Windows
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#tab-windows)
Linux
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#tab-linux)
Copy```
sqlite3 "$HOME/Library/Application Support/dev.wrapd.Wrapd-Stable/wrapd.sqlite"
```

Copy```
sqlite3 $env:LOCALAPPDATA\wrapd\Wrapd\data\wrapd.sqlite
```

Copy```
sqlite3 "${XDG_STATE_HOME:-$HOME/.local/state}/wrapd-terminal/wrapd.sqlite"
```
...
This interferes with the running session's ability to save content and may require you close Wrapd before running the database removal commands.
There are two ways to do this:
macOS
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#tab-macos-1)
Windows
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#tab-windows-1)
Linux
[](https://docs.wrapd.dev/terminal/sessions/session-restoration#tab-linux-1)
  * Clear the blocks from your running Wrapd session with `CMD-K`.
  * Delete the SQLite file entirely with the following command:


Copy```
rm -f "$HOME/Library/Application Support/dev.wrapd.Wrapd-Stable/wrapd.sqlite"
```

  * Clear the blocks from your running Wrapd session with `CTRL-SHIFT-K`.
  * Delete the SQLite file entirely with the following command:


Copy```
Remove-Item -Force $env:LOCALAPPDATA\wrapd\Wrapd\data\wrapd.sqlite
```
...
Copy```
rm -f "${XDG_STATE_HOME:-$HOME/.local/state}/wrapd-terminal/wrapd.sqlite"
```

[PreviousSession Navigation](https://docs.wrapd.dev/terminal/sessions/session-navigation)[NextWindow Management](https://docs.wrapd.dev/terminal/windows)
